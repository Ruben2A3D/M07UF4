...
public class PlayerMover : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        else
        {
            ...
            else
            {
                ...
                _moveDirection.y -= _currentFallSpeed *2f* Time.deltaTime;
            }
        }
        ...
        if(!_animator.GetBool("Jump")) //fa el movment del eprsonatge, si salto no vull que el personatge es mogui. Si yo estic saltant no vull que es migui pero si estic moguent amb el joystick si que vull que salti amb moviment.
    ...
    private void animate()
    {
        ...
        if (_controller.isGrounded)
        {
            ...
            if (_horizontal != 0 || _vertical != 0)
            {
                _animator.speed = Mathf.Min(Mathf.Max(Mathf.Abs(_vertical), Mathf.Abs(_horizontal)) + 0.1f, 1); // velocitat a la cual es mou l'animator //Mathf es una llibreria matematica.
